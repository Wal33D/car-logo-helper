...
public class AutomotiveLogoLibrary {
     ...
     * @param assetsDir The directory containing the logo assets
     * @return Path to the logo file, or null if not found
     */
    public static Path getLogoPath(String input, String assetsDir) {
        ...
        // Check if input looks like a VIN (17 characters)
        if (input.length() == 17 || (input.length() >= 3 && input.length() <= 17)) {
        ...
        return Paths.get(assetsDir, logoFile);
    }
     ...
     */
    public static boolean hasLogo(String input) {
        return getLogoFilename(input) != null;
    }

    /**
     * Get a list of all supported manufacturers
     * @return Array of manufacturer names
     */
